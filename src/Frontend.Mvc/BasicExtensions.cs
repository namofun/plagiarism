...
namespace System
{
    public static class BasicExtensions
    {
        ...
        public static T? AsJson<T>(this string? jsonString) where T : class
        {
        ...
        /// <param name="value">要被序列化的对象。</param>
        /// <returns>序列化后的JSON文本。</returns>
        public static string PrettyJson<T>(this T value)
        {
            return JsonSerializer.Serialize(value,
        ...
        public static string ToHexDigest(this byte[] source, bool lower = false)
        {
            if (source.Length > 4096)
            {
                ...
                var chars = (lower ? "0123456789abcdef" : "0123456789ABCDEF").AsSpan();
            ...
            {
                Span<char> sb = stackalloc char[source.Length * 2];
                var chars = (lower ? "0123456789abcdef" : "0123456789ABCDEF").AsSpan();
                
                for (int i = 0; i < source.Length; i++)
                {
                    sb[i << 1] = chars[(source[i] & 0x0f0) >> 4];
                    sb[i << 1 | 1] = chars[(source[i] & 0x0f) >> 4];
                }
